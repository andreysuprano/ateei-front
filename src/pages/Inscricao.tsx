...
export const Inscricao = () => {
	...
	const handleClick = () => {
		...
		navigate(
			`/customization?nomeEmpresa=${nome}&email=${email}&ordemDeCompra=${ordemDeCompra}&linhas=${formacao.split(
				','
			)[1]}&colunas=${formacao.split(',')[0]}&ledType=${ledType}&produto=${produto}`
		);
	};
